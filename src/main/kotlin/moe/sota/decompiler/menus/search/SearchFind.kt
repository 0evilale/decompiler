...
object SearchFind : FlatMenuItem(), ActionListener {

    init {
        accelerator = KeyStroke.getKeyStroke(KeyEvent.VK_F, Toolkit.getDefaultToolkit().menuShortcutKeyMaskEx)
        mnemonic = KeyEvent.VK_F
        text = LanguageService.getTranslation("search.find")
        addActionListener(this)
    }

    override fun actionPerformed(p0: ActionEvent?) {
        (TabsController.INSTANCE.view.selectedComponent as? TabView)?.showFind()
    }
